...
`docker compose up --build` (repo root) starts `mongo` (with a `mongosh` healthcheck + named volume), `backend` (built from `backend/Dockerfile`, gunicorn, waits for `mongo` healthy), and `frontend` (the root nginx `Dockerfile`). `backend/.dockerignore` keeps `venv`/`.env` out of the image; the root `.dockerignore` excludes `backend` from the frontend build context.

**Base `docker-compose.yml` is production-safe (Coolify-oriented):** it publishes **no host ports** — `mongo` is internal-only, `backend`/`frontend` use `expose`, and a reverse proxy (Coolify's Traefik) routes to them by domain. It is env-driven: `frontend` takes a `VITE_API_BASE_URL` build arg (baked into the bundle — changing it needs a **rebuild**, not a restart) and `backend` takes `CORS_ORIGINS`, both with local-dev defaults.

**`docker-compose.override.yml` is local-only.** Plain `docker compose up` auto-merges it and republishes the host ports (frontend `:8080`, backend `:5000`, mongo `:27017`) so the dev workflow above is unchanged — the SPA hits the default `http://localhost:5000/api` and the browser, running on the host, reaches it. Coolify deploys with an explicit `-f docker-compose.yaml`, which does **not** load the override, so it gets the portless config.

**Production (Coolify), two-domain setup:** give the `frontend` service a domain → port 80 and the `backend` service a domain → port 5000 (in the UI or via `SERVICE_FQDN_FRONTEND_80` / `SERVICE_FQDN_BACKEND_5000`), then set env vars `VITE_API_BASE_URL=https://api.<domain>/api` and `CORS_ORIGINS=https://app.<domain>`, and redeploy with a rebuild. Browser → `app.<domain>` (frontend) → cross-origin calls to `api.<domain>` (backend, CORS-allowed). Do **not** add host `ports:` in the base file — that binds the host directly and conflicts with the proxy (e.g. `Bind for 0.0.0.0:8080 failed: port is already allocated`).
